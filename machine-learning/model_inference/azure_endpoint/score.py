...
from azureml.studio.modules.recommendation.score_svd_recommender.score_svd_recommender import \
    ScoreSVDRecommenderModule, RecommenderPredictionKind, RecommendedItemSelection
...
def run(data):
    ...
    score_params = dict(
        ...
        recommended_item_selection=RecommendedItemSelection.FromAllItems,
        min_recommendation_pool_size=1,
        max_recommended_item_count=20,
        return_ratings=False)
    result_dfd, = ScoreSVDRecommenderModule().run(**score_params)
    result_df = result_dfd.data_frame
    return json.loads(json.dumps({'recommendations': result_df.iloc[0].tolist()[1:]}))
